...
這是一個毫無意義的井字棋項目,它沒有GUI,但它是全英文的所以建議看得懂基礎英文的再下載
...
1. 前往[下載頁面](https://github.com/06712L/idiot-Tic-Tac-Toe/releases/tag/V0.4-beta.1)
2. 下載對應系統的檔案
...
- `make linux` / `make`: 編譯linux可執行檔
- `make win`: 編譯windows可執行檔
- `make cleanlinux`: 清理所有linux編譯產物
- `make cleanwin`: 清理所有windows編譯產物
- `make clean`: 清理所有編譯產物
- `make *** DEBUG=1`: 編譯/清理 除錯版 可執行檔/編譯產物
- `make *** STATIC=1`: 打包靜態庫
